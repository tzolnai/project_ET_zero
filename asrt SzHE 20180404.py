...
def all_settings_def():    
    ...
    try:
        ...
        expstart0.show()
        if expstart0.OK:
            numgroups = expstart0.data[0]
            numsessions = expstart0.data[1]
        else:
            core.quit()
...
def feedback_explicit(rt_m ="", rt_m_p = "", acc_for_p= "", acc_for_the_w= ""):
    ...
    for l in feedback_exp:
        ...
        l = l.replace('*PERCACC*', str(acc_for_the_w)[:6])
...
def feedback_implicit(rt_m = "", acc_for_the_w= ""):
    for i in feedback_imp:
        ...
        i = i.replace('*PERCACC*', str(acc_for_the_w)[:6])
        
        if whether_warning is True:
            print(type(acc_for_the_whole), type(speed_warning))
            print(acc_for_the_whole)
            if acc_for_the_whole > speed_warning:
...
#            # random, implicit
            ...
            try:
                ...
                rt_mean = str(rt_mean)[:5].replace('.',',')
            except:
                rt_mean = 'N/A'
                
            try:
                rt_mean_p =  float( sum(RT_pattern_list)) / len(RT_pattern_list)
                rt_mean_p = str(rt_mean_p)[:5].replace('.',',')
            except:
                rt_mean_p = 'N/A'


            if asrt_types[stim_sessionN[N-1]] == 'explicit':
                whatnow = feedback_explicit(rt_m = rt_mean, rt_m_p = rt_mean_p, acc_for_p = acc_for_patterns, acc_for_the_w = acc_for_the_whole )
            else:
                whatnow = feedback_implicit(rt_mean, acc_for_the_whole)
